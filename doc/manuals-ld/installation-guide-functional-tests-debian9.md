...
Also, the test script uses `netcat` (Netcat) to verify that the broker has started, and `bc` (Basic Calculator) for simple calculations.
Install both of them:
```bash
sudo aptitude install install netcat
