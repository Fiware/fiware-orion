...
char* orionldContextDownload(const char* url, bool* downloadFailedP, OrionldProblemDetails* pdP)
{
  ...
  *downloadFailedP = false;
  ...
  for (int tries = 0; tries < contextDownloadAttempts; tries++)
  {
    ...
    reqOk = orionldRequestSend(&orionldState.httpResponse, protocol, ip, port, "GET", urlPath, contextDownloadTimeout, &pdP->detail, &tryAgain, downloadFailedP, "Accept: application/ld+json", NULL, NULL, 0);
    if (reqOk == true)
      break;
